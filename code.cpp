...
{	//stores back (x,y) values and click type
	//click=1: left click
	//2: right click
	//3: middle click
	union REGS in,out;
...
use
KEYBOARD  after activating keyboard event (use LEFT and RIGHT keys to 
move
the PADDLE
	...
	while ( 1 )
	{
		...
		while ( ! ( ch == 'E' ||   ch == 'P' ) )
		{
			...
			if ( ( ch = getch() ) == 0 )
				...
				ch = toupper ( ch ) ;    /* store the uppercase of the choice 
made*/
		}
